...
**判决（六重证据闭环）**：全部 9 组 E/S/W 地板锁定在 ~76.2/94.3/76.2 m²（极差 ≤0.13 m²，Monte Carlo 噪声量级），与参数化形式（tanh/无界）、锚定强度（0–1e5）、行程约束（硬界/软墙/无）、弯曲能、**init 选择**均无关——naiveinit 组（2026-07-30 补跑，init 逐位等于 B_naive=51.31/77.90/98.33/78.07，真实重力确认）从朴素 LSQ init 端到端同样收敛至 50.42/76.23/94.36/76.16，且总回收量与 comp-init 路径一致（E：77.90→76.23=1.67 ≈ comp 路径 1.19+0.50=1.69）——**两条 init 路径收敛到同一点，地板 init 无关性闭环**。E/S/W 重力地板是 **35 螺栓支撑布局的结构性硬约束**，终稿叙事确立为"认证地板 + 解释地板 + 结构性出路（支撑布局/刚度优化）"。

**剩余工作**：① ~~naiveinit~~（已补跑闭环）；② 110dir 复核任务（`_bound_nograv_300m`、`_fw_tanh_a0_110`，可选）；③ Phase 4：FEA 抽查 + CLAUDE.md 更新（差距三分解表已就位，见下节）。

---

## Phase 4：差距三分解表（终稿核心数据，init 无关性已闭环）

**框架**：每面镜从最差（naive init，重力全罚）到理论最优（B\*，无重力优化下界）的总差距，分解为三段——①闭式补偿 init 回收；②端到端优化再回收；③结构性地板残余（35 螺栓不可达）。

数据 @36dir, 300m NEWS, S95 m²（best_fw 取 a0 基线组；B\* 为 Phase 2 无重力优化下界）：
...
**论文定量核心**：对承罚镜（E/S/W），理论可缩小差距的 **84–87% 是结构性的**——不随参数化、正则、init（naiveinit 组已确认，见上节判决）、优化器而变。螺栓面型调节对重力畸变的补偿上限 ~13–16%。出路只能是支撑布局/刚度设计（支撑点数量与位置、背板结构），这正是论文"结构性出路"一节的定量依据。

**~~待补~~ 已闭环**：naiveinit 四镜终值 50.42/76.23/94.36/76.16，落同一地板（极差 ≤0.13 m²），③占比对 init 选择稳健。

**Phase 4 剩余**：FEA 抽查（取 South 300m 优化终态螺栓 → ANSYS 重算重力畸变 → 与 proxy 对比，确认地板非 proxy 伪影）；CLAUDE.md 实验日志更新；110dir 复核（可选，视时间）。
...
默认角度 0°/29.5°/58.5°，`--compare` 自动对照 proxy 形变。选取依据：South 300m 重力惩罚最大（Δenv≈25 m²）、地板残余占比 84%，若地板系 proxy 伪影将在该镜暴露最明显。判定标准：FEA 与 proxy 的承重力畸变 RMS/shape_corr 与 2026-07-17/21 验证水平一致（RMS~2–3 mm, corr~0.95）→ 地板为真实物理而非 proxy 误差。
