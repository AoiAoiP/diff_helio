...
> **附注（2026-07-30，Phase 5.0 布局扫描后修正）**：上表"高阶(凹陷)"频带的空间来源
> 经区域归因确认为**边缘悬挑带主导**——margin 8% 悬挑区（面积 34%）承载 80–91% 的
> 斜率能量（10°：悬挑 10.18 mrad vs 内部 3.53 mrad）；螺栓间凹陷仅占小头。
> 证据：+80% 螺栓（35→63，跨距 1.8→1.35m）地板仅降 4.2%（`docs/phase5_layout_optimization.md` §0）。
> **本节全部定量结论（PV、斜率预算、可补偿率）不受影响；变化的是机制归因与出路方向：
> 地板的第一结构旋钮是 margin/悬挑与厚度，而非螺栓密度。**
