...
>
> **状态**：Phase 0–4 全部完成（2026-07-30：9 组消融闭环 + FEA 抽查通过 + 110dir 复核通过）。
...
![图 1-1　重力→法线耦合修复前后的光学语义](img/fig_1_1_optical_invisibility.png)

**图 1-1**　修复前（左）：重力只抬高交点高度，法线不变，反射光线与无重力时重合——光学隐身；
修复后（右）：重力进入 `yu/yv`，法线偏转，反射光线偏离开口——重力成为真实光学扰动。
...
> **附注（2026-07-30，Phase 5.0/5.1 布局扫描后修正）**：上表"高阶(凹陷)"频带的空间来源
> 经区域归因确认为**边缘悬挑带主导**——margin 8% 悬挑区（面积 34%）承载 80–91% 的
> 斜率能量（10°：悬挑 10.18 mrad vs 内部 3.53 mrad）；螺栓间凹陷仅占小头。
> 证据：+80% 螺栓（35→63，跨距 1.8→1.35m）地板仅降 4.2%；而 margin 8%→2% + 9×7 组合
> 使地板 −72%（10° 斜率 6.59→1.84 mrad，≈0.84× Buie 太阳宽度）。
...
**图 1-2**　左：重力形变剖面——螺栓处≈0、跨中凹陷，与"过螺栓值的双调和插值"结构性正交；
右：斜率能量三频带分解——高阶（凹陷）分量占绝对主导，螺栓可调张量只能触达仿射+二次尾部。
...
![图 1-3　全年倾角分布、46° 变号点与不可约分量](img/fig_1_3_theta_irreducible.png)

**图 1-3**　左：NEWS 四方位镜的全年镜面倾角 θ 分布——East/West 穿越 46° NLGEOM 变号点；
右：重力场的"年均均值场"（固定螺栓可消去）与"θ 变化分量"（不可约）对比——East/West 的
不可约分量达 2.5–2.9 mrad，任何固定面型都无法补偿。
...
**表 1-1　20 面镜全年倾角范围（110 paper 模式，德令哈 37.36°N，塔高 300 m）**

| 镜位 | 150m | 300m | 600m | 900m | 1200m | 全部距离合并 |
|------|------|------|------|------|-------|-------------|
| North | 20°–49° | 30°–58° | 39°–67° | 43°–72° | 45°–74° | 20°–74°，0% <20° |
| East | 6°–49° | 4°–59° | 4°–68° | 2°–72° | 3°–74° | 2°–74°，**19%** <20° |
| South | 1°–41° | 0°–49° | 1°–60° | 6°–65° | 8°–68° | 0°–68°，**24%** <20° |
| West | 6°–49° | 4°–59° | 4°–68° | 2°–72° | 3°–74° | 2°–74°，**19%** <20° |

> 倾角 θ = 板法向与铅垂方向夹角（0° = 镜面水平朝天，90° = 竖直）。计算基于 110 纸面模式太阳方向，
> 坐标约定同 `generate_sundir_year.py`：x=东，y=天顶，z=南（正=南向）。
> 关键判读：North 全年远离 θ<20° 低角度区（重力弯沉最大）；South 24% 时间恰落此区；
> East/West 19% 时间 θ<20° 且全年穿越 46° NLGEOM 变号点——几何直接预言了 Phase 1
> 实测惩罚图谱（South_150m S95 膨胀 1.539×，North 远镜 ≈1.00×）。
...
**@110dir 复核（§3.8）：③占比 87.4% / 84.0% / 85.6%（E/S/W），与本表 86.9/84.0/86.0% 一致——
结论对太阳方向采样密度稳健。**
...
**@110dir 终稿数字（2026-07-30 复核通过，取代上表作论文终稿；详见 §3.8）**：

| 镜 | B_ideal | B_naive | B_comp | B\* | a0 端到端终值 | 地板残余 a0−B\* |
|---|---|---|---|---|---|---|
| North | 51.58 | 51.63 | 52.02 | **50.06** | 50.62 | 0.56 |
| East | 66.74 | 80.23 | 78.87 | **66.03** | 78.44 | **12.41** |
| South | 72.43 | 96.82 | 93.42 | **71.88** | 92.84 | **20.96** |
| West | 66.92 | 80.49 | 78.98 | **65.92** | 78.39 | **12.47** |
...
- **B_ideal**：理想椭圆面 + 无重力 + 斜率误差的年均 S95。（袁的结果）
- **B\***：无重力下端到端螺栓优化的年均 S95（`disable_gravity:1`）——光学可达性的经验地板。
- **B_naive**：椭圆面直接扔进重力场的年均 S95。
- **B_comp**：闭式补偿计算出一组螺栓后，尝试抵消重力的年均 S95。
...
| `_eval_*_110`（3 个）、`_bound_nograv_300m`（110dir 版）、`_fw_tanh_a0_110` | 110dir 复核（已执行，§3.8） |
...
## 3.8　2026-07-30（二）：110dir 复核——地板非采样伪影，四项判定全过

**动机**（移交文档 `docs/phase4_110dir_desktop_handoff.md`，执行完毕已删）：全部定量结论来自
36dir 训练，而 E/W 镜对稀疏采样过拟合达北侧 4–5 倍（`sundir_sample/EXPERIMENT_REPORT_EAST_WEST.md`）。
若地板在 110dir 下消失/大幅移动，主线垮塌——投稿前必须自己先打的一枪。
台式机执行：3 组 eval + `_bound_nograv_300m`（B\*@110）+ `_fw_tanh_a0_110`（a0@110）。
环境自检通过（bins=12288B、日志 20 行 3-plane 加载、East naive init 80.23 远离幻影签名 65.7）。

**结果**（300m NEWS，S95 m²；括号内为 @36 值）：

| 量 | North | East | South | West |
|---|---|---|---|---|
| B_ideal@110 | 51.58（51.32） | 66.74（65.68） | 72.43（73.51） | 66.92（65.60） |
| B_naive@110 | 51.63（51.31） | 80.23（77.90） | 96.82（98.33） | 80.49（78.07） |
| B_comp@110 | 52.02（51.75） | 78.87（76.71） | 93.42（94.90） | 78.98（76.83） |
| B\*@110 | 50.06（49.77） | 66.03（65.00） | 71.88（73.07） | 65.92（64.68） |
| a0@110 终值 | 50.62（50.34） | 78.44（76.21） | 92.84（94.28） | 78.39（76.19） |

**判定（对照移交文档 §3 证伪线，四项全过）**：

| 检查 | 结果 | 判定 |
|---|---|---|
| B\*@110 vs B\*@36 | Δ = +0.29 / +1.03 / −1.19 / +1.24（N/E/S/W），全在 ±2 m² 内（证伪线 >4） | ✅ |
| 地板残余 a0−B\* | 0.56 / 12.41 / 20.96 / 12.47 vs @36 的 0.57 / 11.21 / 21.21 / 11.51，均 ≥ 2/3 线（消失线 <2） | ✅ |
| 回收率 E/S/W | 12.6% / 16.0% / 14.4%（@36：13.1 / 16.0 / 14.0%），在 10–20% 带内（证伪线 ≥50%） | ✅ |
| 绝对值漂移 | a0：+0.28 / +2.23 / −1.44 / +2.20，<4（E/W 上移 2.2 与已知的 36dir 稀疏采样低估 ~1.7 m² 一致） | ✅ |

**结论**：结构性重力地板在 110dir 下原位不动——E/W 地板甚至略升（11.21→12.41、11.51→12.47），
36dir 若有偏差是**低估**而非高估 E/W 重力严重性。差距三分解 @110：地板占比
E/S/W = 87.4% / 84.0% / 85.6%（@36：86.9 / 84.0 / 86.0%）。**主线在更密采样下成立，
@110 数字取代 @36 为论文终稿数字（§1.5）。**
North 回收率 64%（@36 同为 63%）属预期——免罚镜差距本就在优化噪声量级，不在证伪范围。
