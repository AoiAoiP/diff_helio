...
## 进度日志 2026-07-29：Phase 3 剩余 4 组消融完成——B\* 可达性认证，地板 init 无关

**前置变更**：台式机 `git pull` 拉取 `66d5048`（重力补偿 Phase 0-3 合入），更新了 `data/init_comp_36/`（改进的闭式补偿 init）和 `data_proxy/`（v2 三平面重力格式）。编译通过（Vulkan SDK 1.4.350.0, 28 shader 重编）。

**用户已提前完成**（笔记本，结果待合并）：`_fw_tanh_a1e4`、`_fw_tanh_a1e5`、`_fw_nt_soft1e5`。

**台式机续跑 4 组**（300m NEWS, 36dir, coupling=1, 100 iter, lr=4e-4）：

| 组 | N init→best | E init→best | S init→best | W init→best | 耗时 | 配置特征 |
|---|---|---|---|---|---|---|
| `nt_a1e3_soft1e5` | 52.04→**49.80** | 65.42→**64.98** | 73.73→**73.03** | 65.04→**64.68** | 439s | tanh_bound=0, anchor 1e3, soft 1e5 |
| `nt_a1e3_soft1e6` | →**49.80** | →**64.98** | →**73.03** | →**64.68** | 458s | 同上, soft 1e6（与 1e5 逐位一致） |
| `tanh_a1e3_b1e2` | →**49.83** | →**65.00** | →**73.10** | →**64.65** | 462s | tanh, anchor 1e3, bend 100 |
| `tanh_naiveinit` | 51.32→**49.81** | 65.68→**65.06** | 73.51→**73.08** | 65.60→**64.68** | 437s | tanh, LSQ init, **无锚定无正则** |

**对照基准**（来自 Phase 2 B\* 跑批，无重力优化下界）：

| 镜 | B_ideal (LSQ c0) | B\* (无重力优化) |
|---|---|---|
| North | 51.32 | **49.77** |
| East | 65.68 | **65.00** |
| South | 73.51 | **73.07** |
...
1. **B\* 可达**：全部 4 组、16 镜次均收敛至 B\* ±0.06 m² 以内。锚定正则化（λ=1e3）将优化的引力盆底锚定在斜率空间零重力解附近，消除了 a0 基线中 E/S/W 端到端推不动的行为。

2. **地板 init 无关**（证据链闭环）：`tanh_naiveinit` 使用朴素 LSQ init（无重力补偿、无锚定），100 iter 端到端同样收敛至 B\*。a0 的 "15% 回收率"并非优化器能力不足，而是**旧 comp init 的螺栓配置将优化器困在远离 B\* 的局部极小值**（E/S/W init 76-95 vs B\* 65-73，梯度信号在平台区过弱无法逃离）。

3. **正则形式无关**：tanh vs 物理空间 Adam、软行程墙 vs 弯曲能——四组终值差异 ≤0.07 m²（噪声量级）。锚定项是充分条件，弯曲能/软墙不提供额外增益。

4. **B\* 与 B_ideal 关系**：B\*（无重力优化）在 300m 处仅比 B_ideal（椭圆 LSQ 无重力）低 0.4-1.5 m²。真正战场仍是重力惩罚段——但组合新 comp init + 锚定优化后，**四镜均达到或超越 B\***（E/S/W 的 best < B\* 仅因 B\* 评估使用无重力配置，存在 ~0.02-0.04 m² 的 Monte Carlo 噪声差异）。

**未解释现象**：新 comp init 的 S95（E 65.42, S 73.73, W 65.04）显著优于旧版（E 76.71, S 94.90, W 76.83）。可能原因：`data_proxy/` 重力 bin 在 v2 三平面格式重生成时修正了坐标约定（plate-normal 分量提取），和/或 `lsq_fit_compensated.py` 的闭式补偿公式在 pull 中被修正。需与笔记本旧结果合并后统一核对 init 生成历史。

**待办**：
- [ ] 合并笔记本的 a1e4/a1e5/nt_soft1e5 结果
- [ ] 补全 a1e3 West 镜（本地 log 仅 3/4）
- [ ] 110dir 复核（当前 36dir 可能高估 E/W 侧 ~1.7 m²）
- [ ] 更新 CLAUDE.md 实验日志段

---

## ⚠️ 笔记本侧复核（2026-07-29）：上方"进度日志 2026-07-29"4 组结果全部无效

**判定：台式机运行环境中重力光学失效（幻影重力），上述"B\* 可达 / 地板 init 无关 / 锚定充分"结论不成立。**

证据（笔记本同配置对照 `_fw_nt_a1e3_soft1e5`，重力正常）：笔记本 N 50.39 / E 76.20 / S 94.24 / W 76.19（init 51.75/76.71/94.90/76.83），12527s；台式机同组 49.80/64.98/73.03/64.68（init 52.04/65.42/73.73/65.04），439s。台式机 naiveinit 的 LSQ init 与无重力 B_ideal **逐位相等**（51.32/65.68/73.51/65.60）——重力生效时该值必须为 51.31/77.90/98.33/78.07。

**根因**：v2 三平面重力 bins（12288 B/bin）在笔记本本地重生成后未提交，66d5048 内仍是旧版 4096 B 单平面 bins。台式机 pull 后加载器走 legacy 分支（du/dv=0）→ 法线不受重力斜率影响 → 重力对光斑不可见（即 Phase 1 normal-coupling bug 复活）。请 `grep -m2 "Loaded gravity" logs/_fw_tanh_naiveinit.log` 验证：若显示 `legacy 1-plane` 即坐实。

**处置**：笔记本将提交 v2 bins；台式机 pull 并确认日志显示 `3-plane` 后，原分工任务（`_bound_nograv_300m`、`_fw_tanh_a0_110`、可选重跑 naiveinit）需重跑；本文件 479–513 行的结论与"未解释现象"一节就此闭环（新 comp init 更优之谜=幻影重力下 comp 螺栓无光学代价）。
