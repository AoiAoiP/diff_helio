...
## 文献基准比对（浙大硕士论文 Ch.5 连续曲面型，2026-08-06）

将文献优化的理想多项式面（`data/ellipse.txt`，与论文表 5.4 的 (A,B,C)×10⁻⁴ 逐位一致）经 TPS-LSQ
装配到本管线（`data/init_lsq/`），300m NEWS、36dir、1-iter 纯评估。论文表 5.1 与本管线的口径差：
CSR 0.1 vs 0.01、镜面 12.9×9.6 vs 12.84×9.45、玻璃 3mm/n1.5 vs 4mm/1.523、接收器像素 0.2m vs 0.4m。

**无重力理想多项式面 S95 (m²)**：

| 口径 | North | East | South | West |
|---|:---:|:---:|:---:|:---:|
| **论文值**（用户提供） | 43.38 | 59.03 | 66.60 | 59.03 |
| 本管线生产口径（Buie CSR=0.01, σ_s=1mrad, 157×50px） | 51.33 | 65.69 | 73.50 | 65.61 |
| **文献匹配口径**（Buie CSR=0.01, σ_s=0.85mrad） | **43.16** | **56.14** | **62.96** | **56.07** |
| pillbox, σ_s=1mrad | 49.56 | 61.41 | 67.54 | 61.26 |

**重力开启（m08 + 7×5 布局）多项式面 S95 (m²)**：

| 口径 | North | East | South | West |
|---|:---:|:---:|:---:|:---:|
| 生产口径（σ_s=1mrad） | 51.31 | 77.90 | 98.33 | 78.07 |
| 文献匹配口径（σ_s=0.85mrad） | 43.12 | 68.21 | 87.71 | 68.45 |

**判定：基本一致（无 bug）**。生产口径系统性偏高 +10~18% 完全由口径差解释：太阳形状模型
...
1. Buie 尾翼**能量份额验证正确**（解析积分：CSR=0.1 → 尾翼 10.03%）；尾翼采样链路（反向光追
   以 sunshape 作角度强度核）无采样偏差。
2. **斜率误差方向反常**（CSR=0.1 下 σ_s 1.0→0.5mrad，S95 不降反升 78.8→89.5@S）——
   95% 等高线由环日光晕主导而非镜芯，σ_s 与 CSR 在此口径下不可再分。
3. 残余差异必然位于论文未披露环节：S95 估计器（像素计数 vs 轮廓拟合）、尾翼截断/欠采样
   （其 SPP 远低于我方 2^26 时晕区会欠采样缩小）、或年度平均策略。**结论：论文学名参数
   在我方渲染器不可复现，差异源于实现细节而非配置错误；以 CSR=0.01 生产口径
   （σ_s=0.85mrad 约定）为双方最接近的可比口径（±5.5% 内）。**
...
| `bolt_l1_lambda` | 0.0 | Phase 5.4：螺栓行程 L1 近端（稀疏优化；λ=0 逐位退化） |
| `dump_surface_grad` | 0 | Phase 5.4(A3)：末迭代逐太阳方向面梯度 dump（outputDir/surface_grad_*） |
...
| `docs/phase5_report.md` | **Phase 5 总报告（重写版，2026-08-05）**：螺栓排布（margin→密度→端到端位置）× 材料双轴整合版；真值口径为准 |
