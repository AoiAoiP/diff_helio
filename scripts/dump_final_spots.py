# Final package spot maps (results_final_m04/): flux maps at the 3 validation
# sun directions, bare (zero bolts) vs optimized (truth-reoptimized m04 bolts),
# per mirror. Uses --dump-flux in bolt mode (full physics per sun direction:
# gravity bins interpolated at that sun's tilt + bolt influence), one mirror
# per run so each mirror gets its own optimized bolt set.
#
#   python scripts/dump_final_spots.py
...
def main():
    ...
    tpl = json.load(open(os.path.join(ROOT, "configs/_dump_final_m04.json")))
    ...
    for m in MIRRORS:
        ...
        for case, bolt in [("bare", "data/init_final/zero35.txt"),
                           ("opt", f"data/init_final/m04/{m}_300m_bolt_init.txt")]:
            cfg = dict(tpl)
    ...
    for s in range(N_SUN):
        ...
        for i, m in enumerate(MIRRORS):
            for j, case in enumerate(["bare", "opt"]):
                ...
                im = ax.imshow(f, origin="lower", cmap="hot", vmin=0, vmax=vmax,
                               ...
                               aspect="auto")
                ax.set_title(f"{m} {'bare (zero bolts)' if case == 'bare' else 'optimized m*=0.04'}"
        ...
        fig.suptitle(f"Spot maps at validation sun #{s} (300m, m04 ANSYS-truth gravity)")
